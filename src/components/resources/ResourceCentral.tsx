...
export default function ResourceCentral() {
	...
	const filtered = useMemo(() => {
		const list = resources
			.filter((r) => r.courseCode === activeTab)
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="mt-1 text-sm text-gray-600">
						Access materials uploaded by other students.
					</p>
				</div>
				<div className="text-sm text-gray-600">
					Total: <span className="font-semibold">{resources.length}</span>
				</div>
			</div>
			...
			<div className="mt-4 bg-gray-50 rounded-md p-4 w-full overflow-hidden">
				...
				<div className="flex flex-col gap-4 w-full">
					{pageItems.map((it) => (
						<div
							key={it.id}
							className="flex items-center justify-between gap-3 p-3 bg-yellow-50 rounded-md w-full border border-yellow-100"
						>
							<div className="flex items-center gap-3 min-w-0">
								<div className="p-2 bg-white rounded-md flex items-center justify-center shrink-0 border border-gray-100">
									{iconForType(it.type)}
								</div>
